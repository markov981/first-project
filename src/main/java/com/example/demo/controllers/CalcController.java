...
public class CalcController 
{
		...
		Model                                model) 
	{
		...
		// At this point "the last result from memory" is not defined --> give model the default value
		model.addAttribute("mem", memResult);
		
		
		if (operation.equals("sub")) {
				Subtr sub = new Subtr(first, second);
				model.addAttribute("mem", sub.subtr()); 
				...
				model.addAttribute("mem", exp.expn()); 
			    ...
			    model.addAttribute("mem", memResult);
			    model.addAttribute("oper", "back to clean slate");}			
